...
* **Custom Entity Checkers** — app-specific implementations
e.g. `BorrowerStartTreeChecker` — manages borrower workflow start

Each checker has three core parts:

* **<<listener-registration,Listener Registration>>** (`registerListeners`) — what changes to monitor
...
|Create a listener for changes on a *different* type.
