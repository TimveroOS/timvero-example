...
* **<<listener_registration,Listener Registration>>** (`registerListeners` method) - defines what entity changes to monitor
...
|Creates a listener for entity change events. See <<using_entitychange_method,usage examples>>.

|`entityChange`
|_(no parameters)_
|Monitors the same entity type as the checker (shorthand). See <<using_entitychange_method,usage examples>>.
