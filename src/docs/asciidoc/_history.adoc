...
[source,java,indent=0]
----
include::{sourcedir}/admin/participant/history/ParticipantApplicationStatusChangedHistorySupplier.java[tags=!*]
----

Key components:

* **Parent entity type parameters** - `<UUID, Participant, ...>` - the root entity whose history you're tracking
* **Related entity type parameters** - `<..., UUID, Application>` - the related entity whose fields are being tracked
* **Constructor parameters** - Parent class, related class, and i18n event key
* **`getTrackedFields()`** - Returns fields to monitor in the related entity
* **`getRelatedEntity()`** - Returns the related entity from the parent entity

TIP: Use JPA metamodel constants (e.g., `Application_.STATUS`) for type-safe field references.
...
==== 4. Create History Tab Controller (if not already present)

If you haven't created a History tab for Participant yet:
...
When the Application status changes, the Participant's history timeline will show:

* **Icon** - Edit symbol (MODIFY)
* **Event type** - "Application status changed" (from i18n)
* **Date** - When the status was changed
* **User** - Who changed the status
* **Table** - Showing the field name, old value, and new value

The event appears in the Participant's history even though the change was made to the Application entity.

=== How It Works Internally

The supplier:

1. Queries the audit history of the *related entity* (Application)
2. Filters for changes to the tracked fields only (status)
3. Returns events that are displayed in the *parent entity's* (Participant) timeline
4. Uses the standard `/history/fragments/modified` template

This is different from `AbstractModifiedHistorySupplier` which tracks changes to the root entity itself.

TIP: You can track multiple fields by adding more field names to `getTrackedFields()`. Each change creates a separate event showing all modified fields in that revision.
