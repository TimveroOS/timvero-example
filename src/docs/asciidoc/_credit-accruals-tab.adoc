...
=== Example 1: Interest Accrual Engine
...
=== Example 2: Late Fee Accrual Engine

This example shows implementation for calculating late fees on past due amounts. Late fees use a different base interface (`AccrualEngine` directly, not `RecordableAccrualEngine`) because they only accrue when past-due debt exists — so they are calculated separately and do not appear in the accruals tab.
